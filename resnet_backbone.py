...
def resnet_v1(input_shape, depth, num_classes=10, return_logits=False, input_tensor=None, num_filters=16, return_latent=False, pool_latent=True, skip_downsamples=False, skip_last_downsample=False):
    ...
    """
    if skip_downsamples and skip_last_downsample:
        raise ValueError
    ...
    for stack in range(3):
        for res_block in range(num_res_blocks):
            strides = 1
            if stack > 0 and res_block == 0 and (not skip_downsamples) and (not (skip_last_downsample and stack == 3-1)):  # first layer but not first stack
...
def resnet_feedback_v1(input_shape, depth, num_classes=10, return_logits=False, input_tensor=None, num_filters=16, return_latent=False):
    """feedback version of resnet_v1
    """
    if (depth - 2) % 6 != 0:
        raise ValueError('depth should be 6n+2 (eg 20, 32, 44 in [a])')
    # Start model definition.
    ...
    #second block replaced by feedback connection
    ...
    tensor_shape = deepcopy(input_shape)

    if len(tensor_shape) == 4:
        tensor_x = tensor_shape[1]
        tensor_y = tensor_shape[2]
        tensor_c = num_filters

        tensor_shape = [tensor_x, tensor_y, tensor_c]

    elif len(tensor_shape) == 3:
        tensor_x = tensor_shape[0]
        tensor_y = tensor_shape[1]
        tensor_c = num_filters
    ...
    # Instantiate the stack of residual units with feedback
    for stack in range(3):
        for res_block in range(num_res_blocks):
            strides = 1
            if stack > 0 and res_block == 0:  # first layer but not first stack
            ...
            stack_res_block_input = layers.Input(shape=tensor_shape)
            y = resnet_layer(inputs=stack_res_block_input,
                             num_filters=num_filters,
                             strides=strides)
            y = resnet_layer(inputs=y,
                             num_filters=num_filters,
            ...
            if stack > 0 and res_block == 0:  # first layer but not first stack
                # linear projection residual shortcut connection to match
                # changed dims
                x_internal = resnet_layer(inputs=stack_res_block_input,
                                 num_filters=num_filters,
                                 kernel_size=1,
                                 strides=strides,
                                 activation=None,
                ...
                x_internal = stack_res_block_input
            x_internal = tf.keras.layers.add([x_internal, y])
            stack_res_block_output = layers.Activation('relu')(x_internal)
            stack_res_block = models.Model(inputs=stack_res_block_input, outputs=stack_res_block_output)
            ...
            if stack > 0 and res_block == 0: # first layer but not first stack
                #update input shape to reflect downsampling
                tensor_shape[0] = tensor_shape[0] // 2
                tensor_shape[1] = tensor_shape[1] // 2
            #update input shape to reflect filters output
            tensor_shape[2] = num_filters
...
def ResNet_CIFAR(n=3, version=1, input_shape=(32,32,3), num_classes=10, verbose=True, return_logits=False, num_filters=16, return_latent=False, build_feedback=False, pool_latent=True, skip_downsamples=False, skip_last_downsample=False):
    ...
    if version == 2:
        ...
        if not build_feedback:
            if skip_last_downsample:
                assert(not skip_downsamples)
            model = resnet_v1(input_shape=input_shape, depth=depth, num_classes=num_classes, return_logits=return_logits, num_filters=num_filters, return_latent=return_latent, pool_latent=pool_latent, skip_downsamples=skip_downsamples, skip_last_downsample=skip_last_downsample)
        else:
            if return_latent:
                raise NotImplementedError
            if not pool_latent:
            ...
            if skip_last_downsample:
                raise NotImplementedError
            model = resnet_feedback_v1(input_shape=input_shape, depth=depth, num_classes=num_classes, return_logits=return_logits, num_filters=num_filters, return_latent=return_latent)
